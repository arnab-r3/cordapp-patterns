...
class CrossNotarySwapController {
    ...
    @PostMapping("/token/non-fungible/define", consumes = [MediaType.APPLICATION_JSON_VALUE], produces = [MediaType.TEXT_PLAIN_VALUE])
    private fun defineNonFungibleToken(
        @RequestBody nonFungibleTokenDefinition : CNSForms.NFTDefinition
    ): CompletableFuture<String> {
        require(nonFungibleTokenDefinition.maintainers.isNotEmpty()) {"There should be at least one Non Fungible Token maintainer"}
        logger.info("Request to Non Fungible token with properties: ${nonFungibleTokenDefinition.properties}")


        proxy.startFlowDynamic(NFTFlows.DefineNFTFlow::class.java,
